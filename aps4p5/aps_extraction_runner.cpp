...
test.write_good_points("aps_processed/WMAP/frequentist/gaussLearn_good_points_10k.sav");
...
for(i=0;i<6;i++){
    for(j=i+1;j<6;j++){
        if(i!=3 && j!=3){
            sprintf(outname,"aps_processed/WMAP/bayesian/gaussLearn_%d_%d_10k_bayes.sav",i,j);
            test.draw_bayesian_bounds(outname,i,j,0.95,0.01);
            
            sprintf(outname,"aps_processed/WMAP/frequentist/gaussLearn_%d_%d_10k_frequentist.sav",i,j);
